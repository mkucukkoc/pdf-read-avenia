    ...
    try:
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "word_deep_extract", "error": hexc.detail},
            )
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "word_deep_extract", "error": str(exc)},
            )
        raise HTTPException(
            status_code=500,
            detail={"success": False, "error": "word_deep_extract_failed", "message": msg},
        ) from exc
