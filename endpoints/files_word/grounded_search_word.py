...
logger = logging.getLogger("pdf_read_refresh.files_word.grounded_search")
...
async def grounded_search_word(payload: DocGroundedSearchRequest, request: Request) -> Dict[str, Any]:
    user_id = extract_user_id(request)
    raw_language = payload.language
    language = normalize_language(raw_language) or "English"
    log_full_payload(logger, "word_grounded_search", payload)
    logger.info(
        "Word grounded_search request",
        extra={
    ...
    logger.info("Word grounded_search download start", extra={"chatId": payload.chat_id, "fileUrl": payload.file_url})
    content, mime = download_file(payload.file_url, max_mb=30, require_pdf=False)
    mime = _validate_word_mime(mime)
    logger.info("Word grounded_search download ok", extra={"chatId": payload.chat_id, "size": len(content), "mime": mime})
    ...
    prompt_text = (payload.prompt or "").strip() or f"Answer grounded in the provided document in {language}."
    logger.debug(
        "Word grounded_search prompt | chatId=%s userId=%s lang=%s prompt=%s question=%s",
        payload.chat_id,
        user_id,
        language,
        prompt_text,
    ...
    try:
        logger.info("Word grounded_search upload start", extra={"chatId": payload.chat_id})
        file_uri = upload_to_gemini_files(pdf_bytes, "application/pdf", pdf_filename, gemini_key)
        logger.info("Word grounded_search upload ok", extra={"chatId": payload.chat_id, "fileUri": file_uri})
        parts = [
        ...
        logger.info(
            "Word grounded_search gemini response | chatId=%s preview=%s",
            payload.chat_id,
            text[:500],
        )
        ...
        if firestore_ok:
            ...
            logger.error("Word grounded_search Firestore save failed | chatId=%s", payload.chat_id)
        return result
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "word_grounded_search", "error": hexc.detail},
            )
        logger.error("Word grounded_search HTTPException", exc_info=hexc, extra={"chatId": payload.chat_id})
        raise
    except Exception as exc:
        logger.error("Word grounded_search failed", exc_info=exc)
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "word_grounded_search", "error": str(exc)},
            )
        raise HTTPException(
            status_code=500,
            detail={"success": False, "error": "word_grounded_search_failed", "message": msg},
        ) from exc
