...
logger = logging.getLogger("pdf_read_refresh.files_word.classify")
...
async def classify_word(payload: DocClassifyRequest, request: Request) -> Dict[str, Any]:
    user_id = extract_user_id(request)
    raw_language = payload.language
    language = normalize_language(raw_language) or "English"
    log_full_payload(logger, "word_classify", payload)
    logger.info(
        "Word classify request",
        extra={"chatId": payload.chat_id, "userId": user_id, "language": language, "fileName": payload.file_name},
    )

    logger.info("Word classify download start", extra={"chatId": payload.chat_id, "fileUrl": payload.file_url})
    content, mime = download_file(payload.file_url, max_mb=30, require_pdf=False)
    mime = _validate_word_mime(mime)
    logger.info("Word classify download ok", extra={"chatId": payload.chat_id, "size": len(content), "mime": mime})
    ...
    prompt_text = (payload.prompt or "").strip() or f"Classify this document in {language} using provided labels."
    logger.debug(
        "Word classify prompt | chatId=%s userId=%s lang=%s labels=%s prompt=%s",
        payload.chat_id,
        user_id,
        language,
        payload.labels,
    ...
    try:
        logger.info("Word classify upload start", extra={"chatId": payload.chat_id})
        file_uri = upload_to_gemini_files(pdf_bytes, "application/pdf", pdf_filename, gemini_key)
        logger.info("Word classify upload ok", extra={"chatId": payload.chat_id, "fileUri": file_uri})
        parts = [
        ...
        logger.info(
            "Word classify gemini response | chatId=%s preview=%s",
            payload.chat_id,
            text[:500],
        )
        ...
        if firestore_ok:
            ...
            logger.error("Word classify Firestore save failed | chatId=%s", payload.chat_id)
        return result
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "word_classify", "error": hexc.detail},
            )
        logger.error("Word classify HTTPException", exc_info=hexc, extra={"chatId": payload.chat_id})
        raise
    except Exception as exc:
        logger.error("Word classify failed", exc_info=exc)
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "word_classify", "error": str(exc)},
            )
        raise HTTPException(
            status_code=500,
            detail={"success": False, "error": "word_classify_failed", "message": msg},
        ) from exc
