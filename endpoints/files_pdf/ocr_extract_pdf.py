...
async def ocr_extract_pdf(payload: PdfOcrExtractRequest, request: Request) -> Dict[str, Any]:
    ...
    logger.info(
        "PDF OCR extract request",
        extra={"chatId": payload.chat_id, "userId": user_id, "language": language, "fileName": payload.file_name},
    )

    logger.info("PDF OCR extract download start", extra={"chatId": payload.chat_id, "fileUrl": payload.file_url})
    ...
    prompt = (
        payload.prompt
        or f"Apply OCR if pages are scanned. Extract readable text, tables (as markdown), and key fields. Respond in {language}."
    ).strip()

    try:
        logger.info("PDF OCR extract upload start", extra={"chatId": payload.chat_id})
        file_uri = upload_to_gemini_files(content, mime, payload.file_name or "document.pdf", gemini_key)
        logger.info("PDF OCR extract upload ok", extra={"chatId": payload.chat_id, "fileUri": file_uri})
        ...
        logger.info("PDF OCR extract gemini response | chatId=%s preview=%s", payload.chat_id, extracted[:500])
        ...
        if firestore_ok:
            logger.info("PDF OCR extract Firestore save success | chatId=%s", payload.chat_id)
        else:
            logger.error("PDF OCR extract Firestore save failed | chatId=%s", payload.chat_id)
        return result
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "pdf_ocr_extract", "error": hexc.detail},
            )
        logger.error("PDF OCR extract HTTPException", exc_info=hexc, extra={"chatId": payload.chat_id})
        raise
    except Exception as exc:
        logger.error("PDF OCR extract failed", exc_info=exc)
        ...
        if payload.chat_id:
            save_message_to_firestore(
                ...
                chat_id=payload.chat_id,
                content=msg,
                metadata={"tool": "pdf_ocr_extract", "error": str(exc)},
            )
        raise HTTPException(
            status_code=500,
            detail={"success": False, "error": "pdf_ocr_extract_failed", "message": msg},
        ) from exc
